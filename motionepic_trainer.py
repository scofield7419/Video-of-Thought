...
class MotionEpicTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if not save_flag:
            super(MotionEpicTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        print("Saving model in llava trainer, _save ...")
        if getattr(self.args, 'tune_mm_input_adapter', False):
            super(MotionEpicTrainer, self)._save(output_dir, state_dict)
        else:
            super(MotionEpicTrainer, self)._save(output_dir, state_dict)