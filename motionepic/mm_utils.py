...
def tokenizer_sg_token(prompt, tokenizer, sg_token_index=SG_TOKEN_INDEX, return_tensors=None):
    prompt_chunks = [tokenizer(chunk).input_ids for chunk in prompt.split('<sg>')]
    ...
    for x in insert_separator(prompt_chunks, [sg_token_index] * (offset + 1)):
...
def tokenizer_multiple_token(prompt, tokenizer, sg_token_index=SG_TOKEN_INDEX, video_token_index=VIDEO_TOKEN_INDEX, return_tensors=None):

    input_ids = []
    vid_chunks = prompt.split("<video>")
    for vid_idx, vid_ck in enumerate(vid_chunks):
        _inputs = tokenizer_sg_token(vid_ck, tokenizer, sg_token_index, return_tensors=return_tensors)
            ...
            if DEFAULT_VIDEO_TOKEN in sentence["value"]:
                ...
                if "mmtag" in conversation_lib.default_conversation.version:
                    sentence["value"] = sentence["value"].replace(
                        DEFAULT_VIDEO_TOKEN, "<Video>" + DEFAULT_VIDEO_TOKEN + "</Video>"
                    )
...
def preprocess_llama_2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = True) -> Dict:
    ...
    if has_image:
        input_ids = torch.stack(
            x=[tokenizer_sg_token(prompt, tokenizer, return_tensors="pd") for prompt in conversations], axis=0
        )
    else:
        input_ids = tokenizer(
            conversations,
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            parts[0] += sep
            if has_image:
                round_len = len(tokenizer_sg_token(rou, tokenizer))
                instruction_len = len(tokenizer_sg_token(parts[0], tokenizer)) - 2
            else:
...
def preprocess_plain(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    for source in sources:
        ...
        source[0] = DEFAULT_VIDEO_TOKEN
        conversation = source[0] + source[1] + conversation_lib.default_conversation.sep
        conversations.append(conversation)
    input_ids = [tokenizer_sg_token(prompt, tokenizer, return_tensors="pd") for prompt in conversations]
    targets = copy.deepcopy(input_ids)
    for target, source in zip(targets, sources):
        tokenized_len = len(tokenizer_sg_token(source[0], tokenizer))
