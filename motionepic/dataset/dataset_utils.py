...
from motionepic.constants import IGNORE_INDEX, VIDEO_TOKEN_INDEX, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, MAX_VIDEO_LENGTH
from motionepic.constants import SG_TOKEN_INDEX, MAX_SG_LENGTH, DEFAULT_SG_TOKEN, DEFAULT_SG_START_TOKEN, DEFAULT_SG_END_TOKEN
from motionepic import conversation as conversation_lib
from motionepic.mm_utils import tokenizer_sg_token, tokenizer_multiple_token
    ...
    for source in sources:
        for idx, sentence in enumerate(source):
            ...
            if DEFAULT_SG_TOKEN in sentence['value']:
                if "mmtag" in conversation_lib.default_conversation.version:
                    sentence['value'] = sentence['value'].replace(DEFAULT_SG_TOKEN, '<SG>' + DEFAULT_SG_TOKEN + '</SG>')
                AUDIO_TOKEN_NUM = sentence['value'].count(DEFAULT_SG_TOKEN)
                if AUDIO_TOKEN_NUM > MAX_SG_LENGTH:
                    sentence['value'] = sentence['value'].replace((DEFAULT_SG_TOKEN + ' ') * AUDIO_TOKEN_NUM, DEFAULT_SG_TOKEN * MAX_SG_LENGTH).strip()
                
            replace_token = DEFAULT_SG_TOKEN
            if data_args.mm_use_aud_start_end:
                replace_token = DEFAULT_SG_START_TOKEN + replace_token + DEFAULT_SG_END_TOKEN
            sentence["value"] = sentence["value"].replace(DEFAULT_SG_TOKEN, replace_token)

            if idx % 2 == 1:
                sentence["value"] = sentence["value"].replace(DEFAULT_SG_TOKEN, ' '.join([f"<audio_{i:02d}>" for i in range(data_args.n_aud_tokens)]))
